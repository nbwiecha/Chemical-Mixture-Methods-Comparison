...
filename <- paste("outputs//linear_sims_p", p, "_n", n, "_mcmc20000.Rdata", sep="")
...
out.rho0.linear <- run_simulation_combined(n=n, p=p, np=100, rho=0, eff_type="linear", 
                                           beta1_seq=seq(0, 1, length=5), beta2_seq=seq(0, 1, length=5), B=B, 
                                           hpc=hpc, errorhandling="stop", gam.big=TRUE,
                                           bkmr.iter=20000, bkmr.exact=FALSE)

out.rho5.linear <- run_simulation_combined(n=n, p=p, np=100, rho=.5, eff_type="linear", 
                                                beta1_seq=seq(0, 1, length=5), beta2_seq=seq(0, 1, length=5), B=B, 
                                                hpc=hpc, errorhandling="stop", gam.big=TRUE,
                                              bkmr.iter=20000, bkmr.exact=FALSE)

# out.rho9.linear <- run_simulation_combined(n=n, p=p, rho=.9, eff_type="linear", 
#                                            beta1_seq=seq(0, 1, length=10), beta2_seq=seq(0, 1, length=10), B=B, 
#                                            hpc=hpc, errorhandling="remove", gam.big=FALSE,
#                                            bkmr.iter=20000)

# save results
save(out.rho0.linear,
     out.rho5.linear,
     #out.rho9.linear,
     file=filename)